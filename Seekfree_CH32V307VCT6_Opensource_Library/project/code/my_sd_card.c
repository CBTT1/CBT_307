...
SDHCRES sd_sendcmd(uint8 cmd, uint32 arg, uint8 crc)
{
    ...
    sd_spiReadWrite(crc);
    if(cmd==CMD12)sd_spiReadWrite(0xff);//Skip a stuff byte when stop reading
    //等待响应，或超时退出
    Retry=0X1F;
    do
    {
...
uint8  SD_read_sector_data(uint8 data[], uint32 sector, uint32 count)
{
   ...
   sd_spiReadWrite(0xff);//spi_write_byte(0xff);
   ...
   {
        //等待读响应令牌
        while(sd_spiReadWrite(0xff) != 0xfe);//while(spi_write_byte(0xff) != 0xfe);
        for(i=0; i<512; i++)
        {
          data[add+i] = sd_spiReadWrite(0xff);//data[add+i] = spi_write_byte(0xff);
        }
        //读取CRC码
        sd_spiReadWrite(0xff);//spi_write_byte(0xff);
        sd_spiReadWrite(0xff);//spi_write_byte(0xff);
...
uint8  SD_write_sector_data(uint8 data[], uint32 sector, uint32 count)
{
   ...
   if(count == 1)  //单个扇区数据写入
   {
      ...
      //正常响应应该为0x00
       ...
       sd_spiReadWrite(0xff);//spi_write_byte(0xff);
       //写数据起始令牌
       sd_spiReadWrite(0xff);//spi_write_byte(0xfe);//发送数据块标志0xfe
       for(i=0; i<512; i++)
       {
         sd_spiReadWrite(data[add + i]);//spi_write_byte(data[add + i]);
       }
       //发送CRC码
       sd_spiReadWrite(0xff);//spi_write_byte(0xff);
       sd_spiReadWrite(0xff);//spi_write_byte(0xff);
       for(i=0; ; i++)
       {
         response = sd_spiReadWrite(0xff);//spi_write_byte(0xff);
   ...
   for(j=1; ; j++)
   {
       sd_spiReadWrite(0xff);//spi_write_byte(0xff);
       //写数据起始令牌
       sd_spiReadWrite(0xfc);//spi_write_byte(0xfc);//发送数据块标志0xfc
       for(i=0; i<512; i++)
       {
         sd_spiReadWrite(data[add + i]);//spi_write_byte(data[add + i]);
       }
       ...
       while(sd_spiReadWrite(0xff) != 0xff);//while(spi_write_byte(0xff) != 0xff);
       add += 512;
       if(j == count)
       {
           sd_spiReadWrite(0xFD);//spi_write_byte(0xFD);
           break;
       }
   }
   }
